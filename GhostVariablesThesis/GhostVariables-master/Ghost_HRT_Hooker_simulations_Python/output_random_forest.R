...
npzfile <- np$load("output_simulation/simul_2022_05_10_rf.txt.npz")
...
Imps.hrt.rf.m  = apply(relev_ghost_rank+1,c(2,3),mean)
Imps.hrt.rf.s  = apply(relev_ghost_rank+1,c(2,3),sd)
Imps.hrt.rf.e.m  = apply(relev_ghost_e_rank+1,c(2,3),mean)
Imps.hrt.rf.e.s  = apply(relev_ghost_e_rank+1,c(2,3),sd)
Imps.loco.rf.m = apply(rel_loco_rank+1,c(2,3),mean)
Imps.loco.rf.s = apply(rel_loco_rank+1,c(2,3),sd)
Imps.loco.rf.e.m = apply(rel_loco_e_rank+1,c(2,3),mean)
Imps.loco.rf.e.s = apply(rel_loco_e_rank+1,c(2,3),sd)
rel.gh.rf.m    = apply(relev_ghost_rank+1,c(2,3),mean)
rel.gh.rf.s    = apply(relev_ghost_rank+1,c(2,3),sd)
rel.gh.rf.e.m  = apply(relev_ghost_e_rank+1,c(2,3),mean)
rel.gh.rf.e.s  = apply(relev_ghost_e_rank+1,c(2,3),sd)

rs <- c(0,.9)
nfeat <- 10 

printing = FALSE#TRUE
if (printing) pdf("output_random_forest_Python.pdf", width = 10, height=5)
op <- par(mfrow=c(1,2))
for(k in 1:length(rs)){
  par(mar=c(5,5,4,1))
  plot(Imps.hrt.rf.e.m[k,],type='b',col=2,pch=2,cex=1.5,lwd=2,xlab='Features',ylab='Importance Rank',cex.lab=1.5,cex.axis=1.5,ylim=c(1,nfeat))
  #lines(Imps.loco.rf.m[k,],type='b',col=2,pch=19,cex=1.5,lwd=2)
  lines(Imps.loco.rf.e.m[k,],type='b',col=3,pch=3,cex=1.5,lwd=2)
  #lines(Imps.loco.rf.m[k,],type='b',col=3,pch=19,cex=1.5,lwd=2)
